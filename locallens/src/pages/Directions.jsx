...
export default function Directions() {
  ...
  // On mobile/tablet, re-invalidate after route completes so it fills the container.
  useEffect(() => {
    ...
    return (
      <div style={{ textAlign: 'center', padding: 'var(--space-16)', background: 'var(--ivory)', minHeight: 'calc(100vh - 64px)' }}>
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--ink-soft)' }}>
          ...
          <button
            onClick={() => navigate(-1)}
            style={{ color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', textDecoration: 'underline' }}
          >
            Go back
          </button>
        </p>
  ...
  return (
    <main className="directions-page" style={{ background: 'var(--ivory)', minHeight: '100vh' }}>
      {/* Header */}
      <div
      ...
      {/* Mobile full-bleed map + overlays (desktop can still show side-by-side via CSS if desired) */}
      <div className="directions-fullbleed">
        <div className="directions-map-base">
          <div
            ref={containerRef}
          ...
          {routeStatus === 'error' && (
            <p className="directions-route-error">
              ⚠ Could not find a driving route. OSRM may not cover this area.
            </p>
          )}
        </div>

        <div className="directions-overlay" style={{ pointerEvents: 'none' }}>
          <div className="directions-panel directions-controls" style={{ pointerEvents: 'auto' }}>
            {routeStatus === 'loading' && (
            ...
            <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-5)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
              ...
              {gpsError && (
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--terracotta)', marginBottom: 'var(--space-3)', fontStyle: 'italic' }}>
                  ⚠ {gpsError}
                </p>
              )}
              ...
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', textAlign: 'center', marginBottom: 'var(--space-3)', letterSpacing: '0.08em' }}>
                or type a starting point
              </div>
                ...
                {showOriginDropdown && originSearchResults.length > 0 && (
                  ...
                  >
                    {originSearchResults.map((r, i) => (
                      ...
                      >
                        📍 {r.shortName}
                        <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', fontStyle: 'italic', marginTop: '2px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                          {r.displayName}
                        </span>
                      </button>
                    ))}
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: 'var(--text-xs)', color: 'var(--ink-muted)', textAlign: 'center', lineHeight: 1.6 }}>
